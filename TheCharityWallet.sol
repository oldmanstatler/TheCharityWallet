/*
    ...
    The Charity Wallet is a Clone of the Raven X Charity Token with Certik's Recommended Changes.
    ...
    Twitter: https://twitter.com/_charitywallet
    Website: https://thecharitywallet.com
...
contract TheCharityWallet is Context, IERC20, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = 10**5 * 10**9; //changed to 100,000 from 500,000
    
    address payable public charityAddress = 0x8B99F3660622e21f2910ECCA7fBe51d654a1517D; //Binance Charity Wallet
    ...
    event SetDonationFeePercent (uint256 DonationFee);
    event SetMaxTxPercent (uint256 maxTxPercent, uint256 maxTxDecimals);
    event SwapRouterUpdated(address indexed account, address indexed router, address indexed pair);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        TransferCharityBNB(charityAddress, address(this).balance);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {  //FROM
            _transferFromExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) { //TO
            _transferToExcluded(sender, recipient, amount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) { //BOTH
            _transferBothExcluded(sender, recipient, amount);
        } else {
            _transferStandard(sender, recipient, amount); //Standard
        }
    ...
    function setDonationFeePercent(uint256 DonationFee) external onlyOwner() {
        ...
        emit SetDonationFeePercent(DonationFee);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent, uint256 maxTxDecimals) external onlyOwner() {
        ...
        emit SetMaxTxPercent(maxTxPercent, maxTxDecimals);
    }
